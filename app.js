...
function showEditor() {
  [els.recordsView, els.calendarView, els.statsView, els.settingsView].forEach((item) => {
    item.classList.add("hidden");
  });
...
function setupVoiceInput() {
  ...
  recognition.addEventListener("result", (event) => {
    ...
    for (let index = event.resultIndex; index < event.results.length; index += 1) {
      if (event.results[index].isFinal && index > lastFinalVoiceResult) {
        finalParts.push(event.results[index][0].transcript.trim());
        lastFinalVoiceResult = index;
      }
    }
    const finalText = finalParts.join(" ").trim();
    if (!finalText) return;
    if (finalText === lastFinalVoiceText) return;
    lastFinalVoiceText = finalText;
    const spacer = els.body.value && !els.body.value.endsWith("\n") ? " " : "";
    els.body.value = `${els.body.value}${spacer}${finalText}`;
    scheduleSave();
  });
...
els.back.addEventListener("click", () => showView(currentView));
